...
type L1BeaconEndpointSetup interface {
	Setup(ctx context.Context, log log.Logger) (cl sources.BeaconClient, err error)
...
func (cfg *L1BeaconEndpointConfig) Setup(ctx context.Context, log log.Logger) (cl sources.BeaconClient, err error) {
	a := client.NewBasicHTTPClient(cfg.BeaconAddr, log)
	if cfg.BeaconArchiverAddr == "" {
		return sources.NewBeaconHTTPClient(a), nil
	}
	b := client.NewBasicHTTPClient(cfg.BeaconArchiverAddr, log)
	return sources.NewFallbackClient(a, b, log), nil
}
